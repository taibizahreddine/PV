...
if (isset($_GET['id'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['modifier'])) {
        ...
        $nouveau_gra = $connexion->real_escape_string($_POST['nouveau_gra']);
        $nouvelle_spec = $connexion->real_escape_string($_POST['nouvelle_spec']);
        ...
        $modification_query = "UPDATE informations SET 
            nom = '$nouveau_nom', 
        ...
        if ($resultat->num_rows == 1) {
            ...
            echo "Grade: <select name='nouveau_gra'>";
            echo "<option value='ASST' " . ($row['gra'] == 'ASST' ? 'selected' : '') . ">ASSISTANT</option>";
            echo "<option value='MAB' " . ($row['gra'] == 'MAB' ? 'selected' : '') . ">MAB</option>";
            echo "<option value='MAA' " . ($row['gra'] == 'MAA' ? 'selected' : '') . ">MAA</option>";
            echo "<option value='MCB' " . ($row['gra'] == 'MCB' ? 'selected' : '') . ">MCB</option>";
            echo "</select><br>";
            echo "Specialite: ";
            echo "<input type='radio' name='nouvelle_spec' value='info' " . ($row['spec'] == 'info' ? 'checked' : '') . ">informatique";
            echo "<input type='radio' name='nouvelle_spec' value='eng' " . ($row['spec'] == 'eng' ? 'checked' : '') . "> englais";
            
            echo "<input type='radio' name='nouvelle_spec' value='math' " . ($row['spec'] == 'math' ? 'checked' : '') . "> mathematiques";
           
            echo "<input type='radio' name='nouvelle_spec' value='autre' " . ($row['spec'] == 'autre' ? 'checked' : '') . "> autre<br>";
            ...
            echo "<option value='FR' " . ($row['nat'] == 'ETR' ? 'selected' : '') . ">Etrange</option>";
            ...
            echo "<option value='ETR' " . ($row['cod'] == 'ETR' ? 'selected' : '') . ">France</option>";
            echo "</select><br>";
